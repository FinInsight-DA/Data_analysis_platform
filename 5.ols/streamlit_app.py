...
st.set_page_config(page_title="텍스트 데이터 OLS 분석", layout="wide")
...
with st.form("analysis_form"):
    ...
    with col3:
        ...
        split_year = st.number_input("분할 연도", min_value=1900, max_value=2100, value=2025)
    ...
    if sentiment_df is not None:
        if "company" not in sentiment_df.columns:
            ...
            unique_values = [val for val in all_company_values if val != BOTH_ALIAS][:500]

            if not unique_values:
                st.error("CSV의 `company` 컬럼에서 사용할 수 있는 값이 없습니다.")
                st.stop()

            default_selection = unique_values[:2] if len(unique_values) >= 2 else unique_values
            ...
            if candidate_features:
                ...
                if lag_enabled:
                    ...
                    columns_for_lag_names = (
                        lag_target_columns_selected if lag_target_columns_selected else candidate_features
                    )
                    for col in columns_for_lag_names:
                        for lag in lag_periods_selected:
                            lag_generated_feature_names.append(f"{col}_lag{lag}")
                    lag_generated_feature_names = sorted(set(lag_generated_feature_names))

    selected_feature_names: list[str] = []
    single_feature_choice: str | None = None

    if regression_mode == "단순 회귀" and sentiment_df is not None:
        combined_options = sorted(set(candidate_features + lag_generated_feature_names))
...
if submitted:
    if sentiment_df is None:
        ...
        if lag_enabled and (lag_periods_selected or lag_target_columns_selected):
            lag_periods_effective = sorted({int(p) for p in lag_periods_selected if int(p) > 0})
        ...
        if lag_periods_effective and lag_target_display:
            ...
            st.info(f"시차 변수 적용: {lag_target_display}에 대해 {lag_desc} 기간의 시차를 추가합니다.")

        targets: list[tuple[str, dict]] = []

        if filter_column and filter_values:
            available_companies = set(sentiment_df[filter_column].dropna().astype(str).unique())
            for val in filter_values:
                raw_val = str(val)
                ...
                targets.append(
                    (
                        label,
                        {
                            ...
                            + ([BOTH_ALIAS] if BOTH_ALIAS in available_companies else []),
                            "company_key": raw_val,
                        },
                    )
                )
        ...
        for label, selector in targets:
            try:
                with st.spinner(f"{label} 시장 데이터 수집 중..."):
                    ...
                    market_entries = market_name_inputs.get(market_key, "") if market_key else ""
                    parsed_market_names = [
                        name.strip()
            ...
            if result.diagnostics is not None:
                ...
                st.download_button(
                    ...
                    data=result.diagnostics.to_csv(index=False, encoding="utf-8-sig"),
                ...
                if result.coefficients is not None:
                    ...
                    st.download_button(
                        ...
                        data=result.coefficients.to_csv(index=False, encoding="utf-8-sig"),
                    ...
                    if not sentiment_effects.empty:
                        sentiment_effects["영향 방향"] = sentiment_effects["coef"].apply(
                        ...
                        sentiment_effects["유의성"] = sentiment_effects["p_value"].apply(
                            _significance_label
                        )
                        ...
                        st.dataframe(display_cols.round(6), use_container_width=True)
                        st.caption(
