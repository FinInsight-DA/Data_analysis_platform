...
def main():
    ...
    with col3:
        alpha_input = st.selectbox("Alpha", ['auto', 'symmetric', 0.01, 0.1, 0.5], help="문서-토픽 분포")
        alpha = alpha_input if alpha_input in ['auto', 'symmetric'] else float(alpha_input)
    
    with col4:
        eta_input = st.selectbox("Eta", ['auto', 'symmetric', 0.01, 0.1, 0.5], help="토픽-단어 분포")
        eta = eta_input if eta_input in ['auto', 'symmetric'] else float(eta_input)
